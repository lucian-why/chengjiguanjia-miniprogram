...
function createProfileModule(page) {
  ...
  function selectProfileFromSheet(e) {
    ...
    switchProfileByIndex(index, `Switched to ${page.data.profiles[index].name}`);
  }
  ...
  function confirmAddProfile() {
    ...
    if (!name) {
      wx.showToast({ title: 'Enter profile name', icon: 'none' });
      return;
    }
  ...
  function confirmDeleteProfile(e) {
    ...
    page.setData({
      ...
      confirmMessage: `Delete "${profile.name}" and all exams in it?`,
